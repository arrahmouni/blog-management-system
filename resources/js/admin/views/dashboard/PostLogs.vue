<!-- views/dashboard/PostLogs.vue -->
<template>
    <div class="bg-white rounded-lg shadow-sm p-6">
        <!-- Header -->
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">
                Activity Logs for Post #{{ postId }}
            </h1>
            <button
                @click="goBack"
                class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 cursor-pointer transition-colors"
            >
                Back to Posts
            </button>
        </div>
        ...
        >
            <div
                class="bg-white rounded-lg p-6 w-1/2 max-h-screen overflow-y-auto"
            >
...
<script setup>
    ...
    const {
        ...
        nextPage,
        prevPage,
    } = usePaginationFetcher(`post/${postId}/logs`);
    ...
    const eventBadgeClass = (event) => {
        ...
        return `px-2 py-1 rounded-full text-xs ${
            classes[event] || "bg-gray-100 text-gray-800"
        }`;
    };
